...
import { DEFAULT_PROPERTY_SOURCE, DEFAULT_PROPERTY_SEP } from './constants';
...
export class Property<T> implements IProperty<T> {
    ...
    static from<T>(options: IProperty<T>, target: any, name: string) {
        ...
        if (!property.type) {
            console.warn(`The propery ${name} missing 'type' option and it will be treated as any, you can import module 'reflect-metadata' to get types automatically`);
